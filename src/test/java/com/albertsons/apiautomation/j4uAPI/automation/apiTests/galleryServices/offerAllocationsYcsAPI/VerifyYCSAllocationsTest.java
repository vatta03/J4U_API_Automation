package com.albertsons.apiautomation.j4uAPI.automation.apiTests.galleryServices.offerAllocationsYcsAPI;
 ...
 * Service: Allocations - (YCS)
 * Method: GET
 */
public class VerifyYCSAllocationsTest extends ConfigTestBase {
    ...
    @Test
    public void VerifyYCSAllocationsTest () throws Exception {
        ...
        while (testScriptIterator.hasNext()){
            ...
            if(globalConstants.currentTSDataRowJSONObj.get("execute").toString().equalsIgnoreCase("yes")){
                ...
                appGeneric.assignDetails("Test Script Name: " + testScriptName , "This Test is Currently Runs on API - " + gConstants.currentTestAPI
                        +","+"Current Test Environment:-" + GlobalConstants.testEnvironment + "," + "Current Test API BasePath:=" + currentURL, "Unable to Run the Test");
                appGeneric.generateExtentReport(GlobalConstants.ReportStatus.INFO.toString(), testScriptName);

                //Step 2: Call Allocation Service
                ...
                //Step 4:Get Allocation Service Details
                appGeneric.assignDetails("GET Allocation YCS Details", "Allocation Details are Extracted from GET Response-" + responseDetails, "Unable to Extract Allocation Details from the GET Response");
                appGeneric.generateExtentReport(GlobalConstants.ReportStatus.PASS.toString(), testScriptName);

                //Step 5: Verify Allocation Details
                appGeneric.assignDetails("Verify Allocation in the Service Response", "Correct should displayed in the GET Response: " + responseDetails, "In Correct details is displayed in the GET Response" + responseDetails);
                Assert.assertTrue(responseDetails.contains("offers"),"In Correct Store Id is displayed in the GET Response" + responseDetails);
                appGeneric.generateExtentReport(GlobalConstants.ReportStatus.PASS.toString(), testScriptName);

                //Step 5: Get Headers from the Response
                responseHeaders = getCallWrapper.getAllHeaders(restAssuredObj.baseURI,queryParams,headersParams);
