package com.albertsons.apiautomation.j4uAPI.automation.apiTests.galleryServices.mobileAllocationsMfgAPI;
 ...
 * Service: Mobile Allocations mfg
 * Method: GET
 */
public class VerifyMobileAllocationsMfgServiceTest extends ConfigTestBase {
    ...
    @Test
    public void VerifyMobileAllocationsMfgServiceTest () throws Exception {
        ...
        while (testScriptIterator.hasNext()){
            ...
            if(globalConstants.currentTSDataRowJSONObj.get("execute").toString().equalsIgnoreCase("yes")){
                ...
                headersParams.put("SWY_API_KEY",globalConstants.currentTSDataRowJSONObj.get("SWY_API_KEY").toString());
                ...
                //Step 2:
                //Call Service
                statusCode = getCallWrapper.getStatusCode(restAssuredObj.baseURI,headersParams,headersParams);
                responseDetails = getCallWrapper.getResponseAsString(restAssuredObj.baseURI,headersParams, headersParams);
                ...
                Assert.assertTrue(getCallWrapper.verifyResponseContentType(restAssuredObj.baseURI,headersParams,headersParams),"Response Content Type is not JSON" );
                appGeneric.generateExtentReport(GlobalConstants.ReportStatus.PASS.toString(), testScriptName);
                ...
                appGeneric.assignDetails("Verify Service Response", "Correct details is displayed in the GET Response: " + responseDetails, "In Correct details is displayed in the GET Response" + responseDetails);
                Assert.assertTrue(responseDetails.contains("manufacturerCoupons"),"In Correct Store Id is displayed in the GET Response" + responseDetails);
                appGeneric.generateExtentReport(GlobalConstants.ReportStatus.PASS.toString(), testScriptName);

                //Step 5: Get Headers from the Response
                responseHeaders = getCallWrapper.getAllHeaders(restAssuredObj.baseURI,headersParams, headersParams);
